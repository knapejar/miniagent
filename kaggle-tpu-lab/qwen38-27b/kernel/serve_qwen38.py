...
def server_died(server, phase, **extra):
    ...
    if phase == "failed":
        hold_for_debug()      # tunnels, SSH and /ktl/cmd stay up for a look around
    shutdown(1)
...
while time.time() - t_serve < CFG["keepalive_min"] * 60:
    ...
    if server.poll() is not None:
        ...
        if restarts > CFG["server_restarts"]:
            server_died(server, "failed", step="server-exit")
        cause, block, hint = server_death_report()
        tail = "\n".join(list(server.tail)[-40:])
        log(f"   vLLM exited rc={server.returncode} while serving"
            + (f" — root cause: {cause}" if cause else "") + (f"\n   -> {hint}" if hint else ""))
        log((block or tail)[-4000:])
        publish("server-restart", rc=server.returncode, restart=restarts, of=CFG["server_restarts"],
                cause=cause, hint=hint, tail=(block or tail)[-1500:])
        stop_server(server)
        server = launch_server(CFG)
        startup = wait_healthy(server, CFG, expect_min)
        url = primary_url()

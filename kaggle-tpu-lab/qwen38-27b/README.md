...
cd miniagent/kaggle-tpu-lab                          # launch.py is at the lab root
...
python launch.py serve --no-async-scheduling                     # fallback for JSON mode / structured outputs with MTP on (see "If it fails")
```

### Tunnels, control channels and SSH

The kernel is reachable from its first seconds — over a front server behind several tunnels and
over SSH — while vLLM compiles, after it crashed and while it restarts. If vLLM dies while
serving, the kernel restarts it in place (same TPU session, cached graphs, ~20 min) up to
`server_restarts` times (default 3); if a startup step fails, the session is held for
`--debug-hold-min` (default 30). See [the root README](../README.md#control-channels-tunnels-and-ssh)
...
```
../launch.py                     the CLI at the lab root: serve / status / shell / cmd / ssh / proxy / nettest / stop
...
tools/bench_endpoint.py          TTFT, decode, prefill and 4-stream concurrency through any endpoint
tools/claude-qwen.cmd            Claude Code on this model through `launch.py proxy` (Windows)
```
...
- **The server exits when a client connects, with `AttributeError: __delitem__`**: JSON mode / structured outputs
  while MTP and vLLM's async scheduling are on, a path vllm-tpu 0.28.0 cannot handle. The kernel patches the
  scheduler for it (`KTL fix v2`) and restarts vLLM if it still dies; if it keeps happening, set
  `"async_scheduling": false` (`--no-async-scheduling`) or `"mtp_tokens": 0`.
- **Anything else**: the kernel prints the root cause and the first error block from `vllm.log` when the server
  dies, and holds the session: `python launch.py cmd log 120`, `python launch.py ssh`, and
  [the recovery runbook](../docs/operations.md#4-recovery-runbook).

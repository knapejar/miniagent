...
DEFAULTS = {
    ...
    "tunnel": True,                  # False: no tunnels (local testing)
    "skip_runtime": False,           # True: no pip installs (local testing)
    "port": 8000,                    # what the tunnels point at: the control front server (or the model server without it)
    "control": True,                 # front server + tunnels + SSH from the first seconds (../../common/ktl_control.py);
                                     # False, or the module missing: the model server itself on `port` + one cloudflared tunnel
    "backend_port": 8001,            # the model server behind the front server
    "debug_hold_min": 30,            # on a failure keep the session (and its channels) alive this long
    "net_test": False,               # CPU smoke test of the control plane: no model, dummy server, tunnels, SSH
}
...
CONTROL_B64 = ""  # __CONTROL__  (launch.py embeds common/ktl_control.py here)

FRONT_PORT = int(CFG["port"])
PORT = int(CFG["backend_port"]) if CFG["control"] else FRONT_PORT     # the model server
...
T0 = time.time()
LOG_LINES = []
ktl = None                              # the control module, once loaded
...
def log(*parts):
    ...
    if ktl is not None:
        ktl.raw(line)                   # server.log: what `launch.py cmd log N` shows
...
def publish(phase, **extra):
    ...
    if ktl is not None:
        return ktl.publish(phase, **extra)
...
def fail(step, msg):
    ...
    if ktl is not None and step not in ("no-tpu", "datasets"):   # nothing to look at when the session itself is wrong
        ktl.hold_for_debug()
    sys.exit(1)


def load_control():
    """Front server + tunnels + SSH before anything slow, so a failure later leaves the kernel reachable."""
    global ktl, PORT
    if not (CFG["control"] and CFG["tunnel"]):
        PORT = FRONT_PORT
        return
    if CONTROL_B64 and not CONTROL_B64.startswith("__"):
        with tarfile.open(fileobj=io.BytesIO(base64.b64decode(CONTROL_B64)), mode="r:gz") as tf:
            tf.extractall(WORK)
    for d in (str(WORK), os.path.dirname(os.path.abspath(sys.argv[0] or ".")), os.getcwd()):
        if d not in sys.path:
            sys.path.insert(0, d)
    try:
        import ktl_control as module
    except ImportError:
        PORT = FRONT_PORT
        log("   (no ktl_control module: the server gets one cloudflared tunnel once it is up, no SSH / control routes)")
        return
    module.setup(CFG, log=log, raw_log=WORK / "server.log", port=FRONT_PORT, backend_port=PORT, t0=T0,
                 server_status=lambda: ("serving" if globals().get("srv") else f"not started (phase {module.STATE['phase']})"))
    ktl = module
    ktl.start()
    log(f"   control: front server :{FRONT_PORT} -> model server :{PORT}; tunnels and SSH opening in the background")
...
if CFG["net_test"]:
    if ktl is None:
        sys.exit("net_test needs the control module")
    ktl.run_net_test(CFG["keepalive_min"])
...
if CFG["tunnel"] and ktl is None and not CLOUDFLARED.exists():
...
SCHED.start()
srv = ThreadingHTTPServer(("127.0.0.1" if ktl is not None else "0.0.0.0", PORT), H)
threading.Thread(target=srv.serve_forever, daemon=True).start()
log(f"   HTTP server on :{PORT}" + (f" (behind the front server :{FRONT_PORT})" if ktl is not None else ""))

banner(5, "Tunnel", "the tunnels opened at boot" if ktl is not None else "a public cloudflared URL")
url = None
TUN = [None]
...
if ktl is not None:
    if ktl.BOOT.get("thread"):
        ktl.BOOT["thread"].join(timeout=300)
    summary = ktl.probe_all()
    url = ktl.primary_url()
    publish("tunnel-check", tunnels=summary)
    for n, t in summary.items():
        log(f"   {n:<15} {t['url']}  -> {t['status']}")
elif CFG["tunnel"]:
    url = start_tunnel()
    if url:
...
publish("ready", endpoint=endpoint, api_key=API_KEY, model=MODEL, max_model_len=CFG["max_len"],
        ...
        **({"tunnels": ktl.tunnel_summary()} if ktl is not None else {}))
...
if globals().get("SERVE_FOREVER", True):
    ...
    while time.time() - t_serve < CFG["keepalive_min"] * 60:
        ...
        if SCHED.thread is not None and not SCHED.thread.is_alive():
            if ktl is not None:
                # the engine state is gone with the scheduler; keep the session reachable for a look instead of exiting
                publish("failed", step="scheduler-exit", tail="\n".join(LOG_LINES[-40:])[-2500:])
                ktl.hold_for_debug()
                ktl.shutdown(1)
        ...
        if time.time() - t_beat >= 600:
            t_beat = time.time()
            if ktl is not None:
                endpoint = ktl.primary_url() or endpoint
            publish("heartbeat", up_min=int((time.time() - t_serve) / 60), endpoint=endpoint,
                    requests=STATE["requests"], tokens=STATE["tokens"])
